...
class DocxReader {
    ...
    public function to_html() {
        if ($this->fileData) {
            ...
            foreach ($children->body->children($namespaces['w']) as $child) {

//                $html .= $child->getName() . '<br />';
                if($child->getName() == "p") {
                    $html .= $this->block($child);
                } else if($child->getName() == "tbl") {
                    $html .= "<table>";
                    foreach ($child->tr as $tr) {

                        foreach ($child->tblPr as $tblPr) {

                        }

                        foreach ($child->tblGrid->gridCol as $gridCol) {
                            $html .= "<col";
                            if($gridCol["w"]) { $html .= "width='" . $gridCol["w"] . "'px"; }
                            $html .= " />";
                        }
